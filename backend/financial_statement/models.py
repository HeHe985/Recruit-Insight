...
class FinancialRatio(models.Model):
    # 외래키 작성하는 방법 -> foreignKey 자체가 필드
    corp_code = models.ForeignKey("CorpCode", on_delete=models.CASCADE, db_column="corp_code",related_name="financial_ratio")
    bsns_year = models.IntegerField()
    ...
    # decimal_places=0 : 소수점 없이 정수만 저장하되, Decimal의 정밀도 유지
    thstrm_nm = models.CharField(max_length=10, null=True)
    thstrm_amount = models.DecimalField(max_digits=20, decimal_places=7, null=True)  # 전체 자릿수는 25개, 소수점 5째까지 저장
